...
export default function Settings({ setIsOpen, setting, setSetting, docId }) {
    ...
    async function saveSettingsToDatabase(settings, docId) {
        try {
            await axios.post('/api/saveSettings', { settings, docId });
            console.log('Settings saved to database successfully');
            toast({
                title: "Settings saved to database successfully",                
            })
        } catch (error) {
    ...
    return <div>
        <div className="flex flex-col">
            <label htmlFor="apiKey" className="text-left text-gray-700">OpenAI API Key</label>
